...
const SUBGRAPH_URLS: Record<string, { decentralized: string }> = {
  ...
  "1": {
    decentralized:
      "https://gateway.thegraph.com/api/[api-key]/subgraphs/id/7wzhzJbytdHNsE12B3ascjZKhP8LvoVtuhXqZ97gz2Dr",
  },
};
...
function transformPoolsToTags(chainId: string, pools: Pool[]): ContractTag[] {
  ...
  return validPools.map((pool) => {
    ...
    return {
      ...
      "Project Name": "Stream v2",
      "UI/Website Link": "https://streamprotocol.money/",
      "Public Note": `Stream v2's ${pool.outputToken.symbol} (${pool.outputToken.name}) token contract.`,
    };
  });
}
